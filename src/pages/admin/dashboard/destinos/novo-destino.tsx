...
import { TourDestinationProps, TourDestinations } from '../../../../utils/tourDestinations';
...
const NewDestination = () => {
    ...
    const { saveNewTourDestination, getTourDestinationBySlug } = TourDestinations();
    ...
    const onSubmit: SubmitHandler<TourDestinationProps> = async (data) => {
        ...
        const existSlug = await getTourDestinationBySlug(data.slug);

        if (existSlug.data.length > 0) {
            notifyError('Nome do Local provavelmente já exista. Verifique e tente novamente mais tarde.', 3000);
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            saveNewTourDestination({ setApiValidation, setIsLoading, tourDestination: data });
        } catch (err) {
    ...
    async function handleGetCities(event: React.FormEvent<HTMLSelectElement>) {
        ...
        if (state_id === '') return setCities([]);

        await axiosIBGE
            .get(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${state_id}/municipios?orderBy=nome`)
            .then((response) => {
